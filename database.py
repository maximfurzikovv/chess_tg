...
#     # Получаем состояние игры из таблицы GameState
#     game_state = session.query(GameState).filter_by(room_code=room_code).first()
#     if game_state:
#         return game_state.board_state, game_state.game_in_progress
#     else:
#         print(f"Состояние игры для комнаты {room_code} не найдено.")
#         return None, None
#
#
# def update_game_state(room_code, board_state, game_in_progress):
#     # Получаем текущее состояние игры для данной комнаты
#     game_state = session.query(GameState).filter_by(room_code=room_code).first()
#
#     if game_state:
#         # Обновляем состояние игры
#         game_state.board_state = board_state
#         game_state.game_in_progress = game_in_progress
#         session.commit()
#         print(f"Состояние игры для комнаты {room_code} обновлено.")
#     else:
#         print(f"Состояние игры для комнаты {room_code} не найдено.")
